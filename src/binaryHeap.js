...
function binaryHeap(scoreFunction, idFunction, valueProp) {
    return new BinaryHeap(scoreFunction, idFunction, valueProp);
}

var BinaryHeap = function(scoreFunction, idFunction, valueProp) {
    this.content = [];
...
BinaryHeap.prototype = {
    ...
    push: function(elt) {
        if(this.map[this.idFunction(elt)] !== undefined) {
           // throw 'Error: id "' + this.idFunction(elt) + elt["edgeweight"] +  '" already present in heap';
           // console.log(this.map[this.idFunction(elt)]);
            return;
        }
...
*/
    ...
    _isPrimitive: function (val) {
        if (typeof this.IS_PRIMITIVE !== 'undefined') {
            ...
            this.IS_PRIMITIVE = BinaryHeap._isPrimitive(val);
            return this.IS_PRIMITIVE;
        }
    }
};

BinaryHeap._isPrimitive = function (val) {
